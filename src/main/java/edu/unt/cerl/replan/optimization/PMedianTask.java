 ...
 * @author sarat
 */
public class PMedianTask extends SwingWorkerTask {
    ...
    public PMedianTask(Map<String, String> m, ScenarioState s) {
    ...
    protected Object doInBackground() throws Exception {
        ...
        try {
            ...
            db = new DBInteractions(m, gisConvTools.getPOSTGIS(),
                    gisConvTools.getJDBC_CONNECTION_STRING());
//            db.establishNewConnection();
            //Partitioner partitioner = new Partitioner(db, m,s);

            //performPartitioning(db);
            //
            progressString = "Initializing Optimization.";
            progress += 1;
             ...
             */
            int numSteps = pods.size() - new Integer(m.get(Tables.NUM_PODS));
            if (m.get(Tables.CRITERION).equals(Tables.EXHAUSTIVE)) {
                ...
                if (m.get(Tables.CRITERION).equals(Tables.FIXED)) {
                    ...
                    while (pods.size() > new Integer(m.get(Tables.NUM_PODS))) {
                        ...
                        progressString = "Removing PODs based on condition.";
                        progress += 80 / numSteps;
                        setProgress(Math.min(progress, 100));
                    }
                    ...
                    while (pods.size() > new Integer(m.get(Tables.NUM_PODS))) {
                        this.removePODWithOptima();
                        progressString = "Removing PODs based on selected condition.";
                        progress += 80 / numSteps;
                        setProgress(Math.min(progress, 100));
                    }
                    ...
                    progress += 80 / numSteps;
                    setProgress(Math.min(progress, 100));
                } else if (m.get(Tables.CRITERION).equals(Tables.CENIMBALANCE)) {
                    System.out.println("Centrality Imbalance");
                    while (pods.size() > new Integer(m.get(Tables.NUM_PODS))) {
                        this.removeMaxImbalanceCentralityPOD();
                        progressString = "Removing PODs based on condition.";
                        progress += 80 / numSteps;
                        setProgress(Math.min(progress, 100));
                    }
    ...
    private void performPartitioning(DBInteractions db) {
        //int tempPod = -1;
        //int pod = -1;
        //double distance;
        //DBInteractions db;
        //MultiPolygon[] podsArray = null;
        //ScenarioState s;
        //Map<String, String> m;
        //int population = -1;
        //List pods;
        //Map<Integer, Integer> pop;
        //Map<Integer, Integer> finalDistro;
    ...
    private void removePODBasedOnStandardDeviation() {
        ...
        if (m.get(Tables.CRITERION).equals(Tables.STD_DEV)) {
            optimalValue = (double) ((double) this.population / (double) (pods.size() - 1));
        } else if (m.get(Tables.CRITERION).equals(Tables.STD_DEV_FIXED)) {
            optimalValue = (double) ((double) this.population / (double) (new Integer(m.get(Tables.NUM_PODS))));
        }
    ...
    private void min_black_out() {
        ...
        populationQueue = new PriorityQueue<CensusBlockPopulation>(cbIdHash.size(), populationComparator);
    ...
    public CensusBlockDistance[] Dijkstra(
        ...
        PriorityQueue<CensusBlockDistance> Q =
                new PriorityQueue<CensusBlockDistance>(cbAvailable.length, comparator);
...
//            for( int i=0; i<ua.length; i++ )
    ...
    /*
     * Removing POD with max deviation from optimum removes the POD with most populated or least populated catchment area. Removal of this POD might just make an adjoining catchment area more populated. Further partitioning after the POD removal might actually adversely impact population balance.
     * The approach implemented here, removes that POD which causes the maximum imbalance in population wrt its adjoining catchment areas. Removal of this PODs distributes its population among all adjoining PODs, thus contributing towards removal of PODs that cause the maximum imabalance in POD distribution.
     */

    private void removeMaxImbalanceCentralityPOD() {
//        double optimalValue = 0;
//        if (m.get(Tables.CRITERION).equals(Tables.ADJUSTING_OPTIMA)) {
//            optimalValue = (double) ((double) this.population / (double) (pods.size() - 1));
//        } else if (m.get(Tables.CRITERION).equals(Tables.FIXED)) {
...
//        System.out.println("optimal value = " + optimalValue);
        ...
        while (it.hasNext()) {
            ...
            Map<Integer, Integer> distro = db.populationDistribution(m, pop,
                    values);
            Map<Integer, ArrayList<Integer>> touches = db.neighboringCatchments(m, pop, values, s);
            ...
            while (it2.hasNext()) {
                ...
                if (temp.getId() != pod.getId()) {
                    double currPop = (double) distro.get(temp.getId());
                    double newImbalance = 0;
                    Iterator<Integer> itTouches = touches.get(temp.getId()).iterator();
                    while (itTouches.hasNext()) {
                        double neighPop = (double) distro.get(itTouches.next());
                        newImbalance += (currPop - neighPop);
                    }
                    //double newDeviation = Math.abs(optimalValue - currPop);
                    System.out.println("id = " + temp.getId()
                            + "\t currPop = " + currPop + "\t newImbalance = "
                            + newImbalance);
                    if (Math.abs(newImbalance) > imbalance) {
                        imbalance = Math.abs(newImbalance);
                    }
                }
            }
            if (imbalance < minMaxDeviation) {
                this.finalDistro = distro;
                toRemove = pod;
                minMaxDeviation = imbalance;
            }
            System.out.println("imbalance " + imbalance);
            System.out.println("=======");
        }
        System.out.println("Removing POD " + toRemove.getId()
                + " with imbalance of " + minMaxDeviation);
        pods.remove(toRemove);
    }
    ...
    private void printDistro() {
        ...
        while (it.hasNext()) {
            POD temp = it.next();
            System.out.println(temp.getId() + "\t " + this.finalDistro.get(temp.getId()));
        }
        System.out.println("=====================");
    }
