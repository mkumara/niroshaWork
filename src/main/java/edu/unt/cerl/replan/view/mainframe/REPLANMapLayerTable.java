/*
 *      REPLANMapLayerTable of GEOTools REPLANMapLayerTable that does everything the same but 
 ...
 * index 1 etc. REPLANMapLayerTable stores its layers in the reverse order since it
 ...
 *
 * @source $URL$
 * @version $Id: REPLANMapLayerTable.java,v 1.4 2014-03-18 19:46:56 josh Exp $
 */


public class REPLANMapLayerTable extends JPanel {
     ...
     * will be required.
     */
    public REPLANMapLayerTable() {
        this(null);
    }

    /**
     * Constructor.
     * @param mapPane the map pane this REPLANMapLayerTable will service.
     */
    public REPLANMapLayerTable(MapPane mapPane) {
    ...
    /**
     * Set the map pane that this REPLANMapLayerTable will service.
     *
     ...
     * make up the REPLANMapLayerTable and registers a mouse listener.
     */
    /**
     * Called by the constructor. This method lays out the components that
    ...
    private void initComponents() {
            ...
            public String getToolTipText(MouseEvent e) {
                ...
                if (item >= 0) {
                    ...
                    if (r.contains(e.getPoint())) {
                        ...
                        if (REPLANMapLayerTableCellRenderer.hitSelectionLabel(p)) {
                            //return SELECT_LAYER;

                        } else if (REPLANMapLayerTableCellRenderer.hitVisibilityLabel(p)) {
                            return SHOW_HIDE_LAYER;

                        } else if (REPLANMapLayerTableCellRenderer.hitStyleLabel(p)) {
                            //return STYLE_LAYER;

                        } else if (REPLANMapLayerTableCellRenderer.hitRemoveLabel(p)) {
                            //return REMOVE_LAYER;

                        } else if (REPLANMapLayerTableCellRenderer.hitNameLabel(p)) {
                            //return RENAME_LAYER;
                        }
                    }
                }
        ...
        list.setCellRenderer(new REPLANMapLayerTableCellRenderer());
        list.setFixedCellHeight(REPLANMapLayerTableCellRenderer.getCellHeight());
        ...
        Icon showIcon = REPLANMapLayerTableCellRenderer.LayerControlItem.VISIBLE.getIcon();
        JButton btn = null;
        ...
        Icon hideIcon = REPLANMapLayerTableCellRenderer.LayerControlItem.VISIBLE.getOffIcon();
        ...
        Icon onIcon = REPLANMapLayerTableCellRenderer.LayerControlItem.SELECTED.getIcon();
        ...
        Icon offIcon = REPLANMapLayerTableCellRenderer.LayerControlItem.SELECTED.getOffIcon();
    ...
    private void onReorderLayers(ListDataEvent ev) {
        ...
        for (int pos = ev.getIndex0(); pos <= ev.getIndex1(); pos++) {
            layer = listModel.getElementAt(pos);

            /*
             * REPLANMapLayerTable stores layers in the reverse order to
      ...
      // Refresh the layer that has been moved so it reflects the reposition without
        // having to manually toggle on and off the veiw to update it
